"""Multimodal DenseGNN V3 - Node-level middle fusion.

Based on make_model_asu with node-level text injection (like SGA).
"""
...
def make_model_multimodal_v3(
    ...
    middle_fusion_target: str = "node",  # "node", "global", or "both"
    late_fusion_type: str = "gated",
    late_fusion_cfg: dict = None,
):
    """Make multimodal DenseGNN V3 with node-level middle fusion.

    Args:
        middle_fusion_target: Where to inject text info
            - "node": Inject into node features (SGA-style)
            - "global": Inject into global features (V2-style)
            - "both": Inject into both node and global features
    """
    ...
    if use_middle_fusion:
        for layer_idx in middle_fusion_layers:
            if middle_fusion_target in ["node", "both"]:
            ...
            if middle_fusion_target in ["global", "both"]:
    ...
    for i in range(depth):
        ...
        # Middle fusion: inject text into node/global features
