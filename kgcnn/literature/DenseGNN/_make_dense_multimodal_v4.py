"""Multimodal DenseGNN V4 - Node-level middle fusion without global Dense connection.

Based on V3 but removes global feature Dense connection (like original DenseGNN).
Only node and edge features use Dense connectivity.
"""
...
def make_model_multimodal_v4(
    ...
    middle_fusion_target: str = "node",  # "node" only (no global Dense)
    late_fusion_type: str = "gated",
    late_fusion_cfg: dict = None,
):
    """Make multimodal DenseGNN V4 without global Dense connection.

    Key difference from V3:
        - No Dense connection for global features (ud)
        - Only node and edge features use Dense connectivity
        - middle_fusion_target only supports "node" (global fusion removed)
    """
    ...
    ed = edge_features
    ud = global_features  # No Dense connection for global
    edi = edge_indices
    ...
    # === Middle Fusion Modules (node only) ===
    node_fusion_modules = {}

    if use_middle_fusion and middle_fusion_target == "node":
    ...
    # === DenseGNN Layers (no global Dense connection) ===
    list_embeddings_n = [n]
    list_embeddings_e = [ed]
    # No list_embeddings_u - global features don't use Dense connection

    for i in range(depth):
        if i > 0:
            n = GraphMLP(**gin_mlp)(n)
            ed = GraphMLP(**gin_mlp)(ed)
            # No GraphMLP for ud - keep original dimension

        # Middle fusion: inject text into node features only
        if use_middle_fusion and i in middle_fusion_layers:
            if middle_fusion_target == "node":
        ...
        # No list_embeddings_u.append(up)

        n = LazyConcatenate()(list_embeddings_n)
        ed = LazyConcatenate()(list_embeddings_e)
        ud = up  # Just use current layer output, no Dense connection
