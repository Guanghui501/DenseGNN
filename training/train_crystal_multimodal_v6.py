"""Training script for Multimodal DenseGNN V5 (graph structure identical to original DenseGNN).

Usage:
    python train_crystal_multimodal_v6.py \
        --hyper hyper/hyper_multimodal_v5.py \
        --category DenseGNN_Multimodal_V5 \
...
def build_model(hyper, text_dim=768):
    """Build multimodal DenseGNN V5."""
    model_cfg = hyper["model"]["config"]

    # Add text_embedding to inputs
    ...
    model = make_model_multimodal_v5(
        inputs=inputs,
        name=model_cfg.get("name", "DenseGNN_Multimodal_V5"),
        ...
        output_mlp=model_cfg.get("output_mlp", {}),
        g_pooling_args=model_cfg.get("g_pooling_args", {}),
...
def main():
    ...
    parser.add_argument("--split", type=str, default=None, help="Train/val/test split ratio, e.g., '8:1:1'")
    parser.add_argument("--seed", default=42, type=int, help="Seed for data split")
    parser.add_argument("--text_embeddings", default=None)
    parser.add_argument("--random_text_emb", action="store_true")
    parser.add_argument("--text_dim", type=int, default=768)
    parser.add_argument("--test", action="store_true", help="Test mode: 100 samples, 3 epochs")
    args = parser.parse_args()

    print("=" * 60)
    print("Multimodal DenseGNN V5 (Graph identical to original DenseGNN)")
    print("=" * 60)
    print(f"Args: {args}")
    ...
    print("\nLoading dataset...")
    # Support both "dataset" and "data.dataset" structures
    ...
    for fold_idx, split in enumerate(splits):
        ...
        if args.split:
            ...
            val_idx = test_idx  # Use test as validation for cross-validation
        print("=" * 60)

        model = build_model(hyper, text_dim=text_dim)

        # Input order: edge_inputs + node_inputs + [edge_indices, text_input]
        # No charge input (same as original DenseGNN)
        input_order = ['offset', 'voronoi_ridge_area', 'atomic_number', 'AGNIFinger', 'edge_indices']
        input_list = [original_inputs[k] for k in input_order]
        ...
        print(f"Graph inputs: {len(x_train_graph)}")

        x_train = x_train_graph + [text_embeddings[train_idx]]
        x_val = x_val_graph + [text_embeddings[val_idx]]
        x_test = x_test_graph + [text_embeddings[test_idx]]
        ...
        model_name = hyper["model"]["config"].get("name", "DenseGNN_Multimodal_V5")
        if "dataset" in hyper._hyper:
        ...
        axes[1].set_ylabel('MAE (GPa)')
        axes[1].set_title(f'Fold {fold_idx} - MAE (Best: {mae_val:.2f} GPa)')
        axes[1].legend()
        axes[1].grid(True)
